...
class JiraAgent(BaseLangGraphAgent):
    ...
    SYSTEM_INSTRUCTION = scope_limited_agent_instruction(
        ...
        additional_guidelines=[
            ...
            "MANDATORY: ALWAYS send the JQL query back to the user showing exactly what was searched",
            "After executing a search, explicitly state: 'Searched with JQL: [your JQL query]' before presenting results",
            "MANDATORY: If no limit is specified in a search request, default to retrieving 100 jira issues (use limit=100 parameter)",
            "MANDATORY: If a simple JQL query fails or returns no results, automatically retry with a text search using the 'text ~' operator",
            "For text search fallback, convert the search terms to: text ~ \"[search terms]\" and combine with any existing filters",
            ...
            "1. ALWAYS include the current date and time of day at the beginning of search results",
            "2. Format the time display as: 'Current date/time: [YYYY-MM-DD HH:MM:SS]' or 'Current date/time: [current date and time]'",
            ...
            "5. When user requests 'tabulate' or 'table', ALWAYS format data as a markdown table",
            ...
            "Current date/time: 2025-01-15 14:30:00",
