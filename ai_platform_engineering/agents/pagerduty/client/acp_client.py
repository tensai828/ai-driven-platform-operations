...
async def run_stateless(question: ChatBotQuestion, process_event):
  """
  Create a stateless run with the input spec from pagerduty-agent.json and stream the output.
  ...
  async with AsyncApiClient(client_config) as api_client:
    ...
    # Compose input according to the input spec in pagerduty-agent.json
    input_obj = {
      "pagerduty_input": {
        "messages": [
          {
