...
class TestRAGEndToEnd:
    ...
    def test_url_ingestion_workflow(self):
        ...
        # Test URL - using local httpbin for reliable test content
    ...
    def test_datasource_listing(self):
        ...
        # Store first datasource ID for later tests
        self.test_datasource_id = datasources[0]["datasource_id"]
    ...
    def test_query_ingested_content(self):
        ...
        # Query for content that should be in the httpbin HTML page
        query_data = {
            "query": "HTML page",
            "limit": 5,
    ...
    def test_datasource_reload(self):
        ...
        # Fetch datasource list and grab the first one
        response = requests.get(f"{self.base_url}/v1/datasources")
        assert response.status_code == 200, f"Failed to list datasources: {response.text}"
        
        datasources_data = response.json()
        if datasources_data["count"] == 0:
            pytest.skip("No datasources available for reload test")
        
        datasources = datasources_data["datasources"]
        test_datasource_id = datasources[0]["datasource_id"]
    ...
    def test_datasource_cleanup(self):
        ...
        # Fetch datasource list and grab the first one for deletion
        response = requests.get(f"{self.base_url}/v1/datasources")
        assert response.status_code == 200, f"Failed to list datasources: {response.text}"
        
        datasources_data = response.json()
        if datasources_data["count"] == 0:
            pytest.skip("No datasources available for cleanup test")
