...
class AgentState(BaseModel):
    ...
    gitlab_input: InputState = Field(default_factory=InputState, description="Input state to the agent")
    gitlab_output: Optional[OutputState] = Field(default=None, description="Output state from the agent")
    ...
    def safe_model_dump(self) -> Dict[str, Any]:
        ...
        if "metadata" in data and "env" in data["metadata"]:
            ...
            if "GITLAB_PERSONAL_ACCESS_TOKEN" in data["metadata"]["env"]:
                data["metadata"]["env"]["GITLAB_PERSONAL_ACCESS_TOKEN"] = "***REDACTED***"
            if "GITLAB_OAUTH_TOKEN" in data["metadata"]["env"]:
                data["metadata"]["env"]["GITLAB_OAUTH_TOKEN"] = "***REDACTED***"
            if "AWS_ACCESS_KEY_ID" in data["metadata"]["env"]:
                data["metadata"]["env"]["AWS_ACCESS_KEY_ID"] = "***REDACTED***"
            if "AWS_SECRET_ACCESS_KEY" in data["metadata"]["env"]:
                data["metadata"]["env"]["AWS_SECRET_ACCESS_KEY"] = "***REDACTED***"
        return data
