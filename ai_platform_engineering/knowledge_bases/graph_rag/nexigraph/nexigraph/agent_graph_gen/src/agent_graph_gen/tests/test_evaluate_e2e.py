...
async def test_each_evaluation():
    ...
    for i in range(number_of_tests):
        ...
        agent = ForeignKeyRelationAgent(rc.acceptance_threshold, rc.rejection_threshold, 1, 600)
        ...
        for rel_id, ground_truth_candidate in ground_truth_candidates.items():
            ...
            test_confidence = test_candidates[rel_id].evaluation.relation_confidence
        ...
        assert (tp + tn) / len(ground_truth_candidates) >= 0.7, f"Expected at least 70% of the candidates to be evaluated correctly, but got {(tp + tn) / len(ground_truth_candidates)}"
