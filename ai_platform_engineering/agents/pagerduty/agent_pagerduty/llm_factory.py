...
# Optional imports - only used when the specific provider is selected
try:
...
class LLMFactory:
  ...
  def get_llm(
    ...
    *provider*.
    """
    # Check if the required module is available for the selected provider
    if self.provider == "anthropic_claude" and not ANTHROPIC_AVAILABLE:
        logging.warning("Anthropic Claude provider selected but langchain_anthropic module not available. Falling back to Azure OpenAI.")
        self.provider = "azure_openai"
    
    if self.provider == "google_gemini" and not GOOGLE_GENAI_AVAILABLE:
        logging.warning("Google Gemini provider selected but langchain_google_genai module not available. Falling back to Azure OpenAI.")
        self.provider = "azure_openai"
