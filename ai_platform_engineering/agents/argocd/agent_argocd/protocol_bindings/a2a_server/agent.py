...
_prompt_config = load_subagent_prompt_config("argocd")
...
class ArgoCDAgent(BaseLangGraphAgent):
    ...
    async def stream(self, query: str, sessionId: str, trace_id: str = None):
        """
        Stream responses with argocd-specific tracing and safety-net error handling.
        ...
        try:
            ...
            logger.error(f"Unexpected ArgoCD agent error: {str(e)}", exc_info=True)
            yield {
                'is_task_complete': True,
                'require_user_input': False,
                'kind': 'error',
                'content': f"❌ An unexpected error occurred in ArgoCD: {str(e)}\n\nPlease try again or contact support if the issue persists.",
            }
