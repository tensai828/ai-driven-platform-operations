...
class ArgoCDAgent(BaseLangGraphAgent):
    ...
    SYSTEM_INSTRUCTION = build_system_instruction(
        ...
        response_guidelines=SCOPE_LIMITED_GUIDELINES + STANDARD_RESPONSE_GUIDELINES + [
            ...
            "",
            "**CRITICAL - Multi-Strategy Search Approach**:",
            "When searching for resources, especially with ambiguous or short queries, you MUST try multiple search strategies:",
            "",
            "1. **Primary Search Strategy - Use Search_Argocd_Resources tool**:",
            "   - The Search_Argocd_Resources tool searches across multiple fields simultaneously:",
            ...
            "   - Can search across multiple resource types at once",
            "",
            "2. **Search Strategy Examples**:",
            "   - 'show apps for dir' → Try:",
            "     a) Search_Argocd_Resources(query='dir', resource_types=['applications'])",
            "     b) If no results, try broader: Search_Argocd_Resources(query='dir', resource_types=['applications', 'projects'])",
            "     c) Try variations: Search_Argocd_Resources(query='directory', resource_types=['applications'])",
            "     d) Try path-based: Search_Argocd_Resources(query='dir', resource_types=['applications']) - searches in paths too",
            "",
            "   - 'find prod apps' → Try:",
            "     a) Search_Argocd_Resources(query='prod', resource_types=['applications'])",
            "     b) If too many results, refine: Search_Argocd_Resources(query='production', resource_types=['applications'])",
            "",
            "   - 'apps with staging' → Try:",
            "     a) Search_Argocd_Resources(query='staging', resource_types=['applications'])",
            "     b) Searches in names, labels, annotations, descriptions simultaneously",
            "",
            "3. **Fallback Strategies When Initial Search Returns No Results**:",
            "   If Search_Argocd_Resources returns no results, try these approaches:",
            "",
            "   a) **Try Different Query Variations**:",
            "      - Short query 'dir' → Try 'directory', 'directories', 'dirs'",
            "      - Partial match 'prod' → Try 'production', 'prod-', '-prod'",
            "      - Abbreviation 'dev' → Try 'development', 'dev-', '-dev'",
            "",
            "   b) **Try Broader Resource Types**:",
            "      - If searching only 'applications', try including 'projects' and 'applicationsets'",
            "      - Example: Search_Argocd_Resources(query='dir', resource_types=['applications', 'projects', 'applicationsets'])",
            "",
            "   c) **Try List_Applications with Filters**:",
            "      - If search fails, try List_Applications with name filter (exact match)",
            "      - Example: List_Applications(name='dir') or List_Applications(project='dir')",
            "      - Note: List_Applications name filter is exact match, so use for known names",
            "",
            "   d) **Search in Different Fields**:",
            "      - The Search_Argocd_Resources tool automatically searches in:",
            "        * Application names, paths, repos, namespaces, projects",
            "        * Project names, descriptions, source repos",
            "        * Cluster names, servers, labels, annotations",
            "      - If one field doesn't match, other fields might",
            "",
            "4. **Handling Ambiguous Queries**:",
            "   For queries like 'dir', 'test', 'dev' that could match multiple things:",
            "",
            "   a) **Start Broad, Then Narrow**:",
            "      - First: Search_Argocd_Resources(query='dir', resource_types=['applications', 'projects', 'applicationsets'])",
            "      - If results found, show them and ask user to clarify",
            "      - If no results, try variations (directory, dirs, etc.)",
            "",
            "   b) **Provide Contextual Suggestions**:",
            "      - If search returns no results, suggest:",
            "        * 'No exact matches found. Did you mean:'",
            "        * 'Try searching for: directory, directories, or list all applications to browse'",
            "        * 'You can also filter by project, namespace, or repository'",
            "",
            "   c) **Use Multiple Search Attempts**:",
            "      - Don't give up after one search",
            "      - Try 2-3 variations before concluding no results",
            "      - Example sequence:",
            "        1. Search_Argocd_Resources(query='dir', resource_types=['applications'])",
            "        2. If empty: Search_Argocd_Resources(query='directory', resource_types=['applications'])",
            "        3. If empty: Search_Argocd_Resources(query='dir', resource_types=['applications', 'projects'])",
            "",
            "5. **Search Best Practices**:",
            "   - Always use case_sensitive=False (default) for fuzzy matching",
            "   - For short queries (1-3 chars), try multiple variations",
            "   - For partial words, the search tool supports substring matching automatically",
            "   - Combine multiple search terms by trying each separately if needed",
            "   - Use resource_types parameter to narrow search scope when appropriate",
            "",
            "6. **Search Tool Parameters**:",
            "   - query (str): Search term - supports partial matches, case-insensitive by default",
            "   - resource_types (List[str]): Filter by type - ['applications', 'projects', 'applicationsets', 'clusters']",
            "   - case_sensitive (bool): Default False for fuzzy matching",
            "   - page/page_size: Use pagination for large result sets",
            "",
