...
# Find installed path of the jira_mcp sub-module
spec = importlib.util.find_spec("agent_jira.protocol_bindings.mcp_server.mcp_jira.server")
if not spec or not spec.origin:
    raise ImportError("Cannot find agent_jira.protocol_bindings.mcp_server.mcp_jira.server module")
...
async def create_agent(prompt=None, response_format=None):
  ...
  # Find installed path of the jira_mcp sub-module
  spec = importlib.util.find_spec("agent_jira.protocol_bindings.mcp_server.server")
  if not spec or not spec.origin:
      raise ImportError("Cannot find agent_jira.protocol_bindings.mcp_server.server module")
...
# Setup the Jira MCP Client and create React Agent
async def _async_jira_agent(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if state.jira_input is None:
        ...
        if isinstance(input_data, dict) and "messages" in input_data:
            ...
            messages = []
    elif hasattr(state.jira_input, "messages"):
        messages = getattr(state.jira_input, "messages")
    elif isinstance(state.jira_input, dict) and "messages" in state.jira_input:
        messages = [Message.model_validate(m) for m in state.jira_input["messages"]]
    else:
        messages = []
