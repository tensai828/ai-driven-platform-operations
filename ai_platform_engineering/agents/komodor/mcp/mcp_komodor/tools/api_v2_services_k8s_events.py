...
async def post_api_v2_svcs_k8s_events(
  body_event_type: str,
  body_summary: str,
  ...
  """
  Create a custom k8s event for services

  OpenAPI Description:
      Create a new custom event that will be shown in the timeline and events list. The event will be associated with the specified services.
  ...
  Args:
      ...
      body_scope_details (Dict[str, Any]): Optional. Additional key-value pairs for extra event details.
  ...
  """
  logger.debug("Making POST request to /api/v2/services/k8s-events")
  ...
  if body_scope_details is not None:
    flat_body["scope_details"] = body_scope_details
  data = assemble_nested_body(flat_body)

  success, response = await make_api_request("/api/v2/services/k8s-events", method="POST", params=params, data=data)
