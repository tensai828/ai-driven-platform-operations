...
2. Find the properties of the entity types using `get_entity_properties` tool.
3. Once the entity_types and its properties are established, and the query does not ask for specific entities or relations, use `raw_query` directly.
4. If there are specific values, use the fuzzy_search tool to find entities related to the question. The fuzzy_search is limited to a maximum of 100 results, use `raw_query` to get more results if needed.
5. If query has multiple entity types, Use the `get_relation_path_between_entity_types` tool to find both direct AND indirect relations between the entity types.
...
7. If the raw_query returns no results, use the fuzzy_search again with different parameters.
