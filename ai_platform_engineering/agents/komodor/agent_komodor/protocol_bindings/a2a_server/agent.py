...
class KomodorAgent(BaseLangGraphAgent):
    ...
    async def stream(self, query: str, sessionId: str, trace_id: str = None):
        """
        Stream responses with komodor-specific tracing and safety-net error handling.
        ...
        try:
            ...
            logger.error(f"Unexpected Komodor agent error: {str(e)}", exc_info=True)
            yield {
                'is_task_complete': True,
                'require_user_input': False,
                'kind': 'error',
                'content': f"❌ An unexpected error occurred in Komodor: {str(e)}\n\nPlease try again or contact support if the issue persists.",
            }
