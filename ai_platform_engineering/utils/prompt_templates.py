...
MARKDOWN_FORMATTING_CHECKLIST = [
    "**CRITICAL**: ALWAYS ensure your final output is properly formatted markdown without errors",
    "Before returning ANY response, validate markdown formatting:",
    "  ✓ Tables: Properly formatted with `|` separators, headers, and alignment rows",
    "  ✓ Spacing: Blank lines before/after tables, lists, and code blocks",
    "  ✓ Headers: Use `#` for titles, `##` for sections (never broken across lines)",
    "  ✓ Lists: Consistent bullet/number formatting with proper indentation",
    "  ✓ Code blocks: Use triple backticks ``` with language tags",
    "  ✓ Links: Proper format `[text](url)` without broken syntax",
    "  ✓ No text concatenation: Each element separated by proper whitespace",
    "",
    "**TABLE FORMATTING RULES**:",
    "- First line: Table header with `|` separators",
    "- Second line: Alignment row with `|---|---|---|` pattern",
    "- Data rows: One per line with `|` separators",
    ...
    "- ❌ Broken table rows (missing `|` or alignment)",
    "- ❌ Headers split across multiple lines",
    "- ❌ Missing blank lines around formatted elements",
    "- ❌ Malformed JSON in code blocks",
    "- ❌ Inconsistent list formatting",
    "",
    "**VALIDATION CHECKLIST** (Check before EVERY response):",
    "1. Are tables properly formatted with headers and alignment?",
    "2. Is there whitespace between different sections?",
    "3. Are all lists using consistent bullets/numbers?",
    "4. Do code blocks have proper opening/closing backticks?",
    "5. Can this markdown be rendered without errors?"
]
