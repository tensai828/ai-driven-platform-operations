...
class KomodorAgent(BaseLangGraphAgent):
    ...
    SYSTEM_INSTRUCTION = build_system_instruction(
        ...
        response_guidelines=SCOPE_LIMITED_GUIDELINES + STANDARD_RESPONSE_GUIDELINES + [
            ...
            (
                "CLUSTER NAME WORKFLOW: If the user provides a service/application name but no cluster name: "
                "(1) First call post_api_v2_svcs_search WITHOUT body_scope_cluster to search across all clusters, "
                "(2) Extract the cluster name from the response, "
                "(3) Use that cluster name for subsequent operations (health checks, events, RCA). "
                "If the user provides the cluster name directly, use it immediately. "
                "If neither service name nor cluster name is provided, ask the user for at least one of them"
            )
        ],
