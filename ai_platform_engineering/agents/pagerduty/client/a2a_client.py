...
async def run_single_turn_test(client: A2AClient) -> None:
    ...
    send_payload = create_send_message_payload(
        text='List all active incidents in PagerDuty',
    )
...
async def run_streaming_test(client: A2AClient) -> None:
  ...
  send_payload = create_send_message_payload(
    text='Who is the ETI SRE PRIMARY for 20th may 2025',
  )
...
async def run_multi_turn_test(client: A2AClient) -> None:
  """Runs a multi-turn test about PagerDuty incidents and their status, streaming output like run_streaming_test."""
  print('--- Multi-Turn PagerDuty Incident Status Request ---')

  # --- First Turn ---
  first_turn_payload = create_send_message_payload(
    text='List all high urgency incidents in PagerDuty.'
  )
