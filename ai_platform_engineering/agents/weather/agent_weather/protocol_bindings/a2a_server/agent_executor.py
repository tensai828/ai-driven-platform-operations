...
class WeatherAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        # Extract trace_id from A2A context - Weather is a SUB-AGENT, should NEVER generate trace_id
        trace_id = extract_trace_id_from_context(context)
        if not trace_id:
            logger.warning("Weather Agent: No trace_id from supervisor")
            trace_id = None
        else:
            logger.info(f"Weather Agent: Using trace_id from supervisor: {trace_id}")
